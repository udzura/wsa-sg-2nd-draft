...
これらの前段のデーモンは本来rootユーザ以外で立ち上がることが望ましく、先述した非特権コンテナは利用できる状況が限定されるので、setuid rootされたコンテナランタイムを利用したい。そこで、suEXECセキュリティモデルのような権限の検査が有効であると考える。
...
<cite>図5: mrubyにおけるビルドオプションDSL。定数のdefineなどを簡潔・動的に記述できる。Haconiwaも同等のものを利用する</cite>
